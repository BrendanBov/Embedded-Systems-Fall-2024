...
int main(){
	...
	while(runFlag){
		...
		if (eventRaised)
			if (isButton)
				stopMovement();	// checks for movement stop
			
		movement(currSpeed, currRad);
...
void setSpeedAndRad()
{
	if(eventRaised && isJoystick)
	{
		if(inputNumber == joy1Y)
		{
			currSpeed = inputValue / 100;
		}
		if(inputNumber == joy2Y)
		{
			currRad = inputValue / 100;
		}
	}
}
